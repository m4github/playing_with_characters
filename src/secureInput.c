/**
 * @file srcureInput.h
 * @author Mohadeseh_Forghani (forghani_m@office.zharfpouyan.net)
...
initMemory (struct input *data)
{
  ...
  MALLOC_ERROR_CHECK (data->word, MAX_LEN);

  data->sentence = (char *)malloc (sizeof (char) * MAX_LEN);

  MALLOC_ERROR_CHECK (data->sentence, MAX_LEN);
...
getCharacter (struct input *data)
{
  ...
  MALLOC_ERROR_CHECK (tmp, sizeof (char));
...
getWord (struct input *data)
{
  ...
  MALLOC_ERROR_CHECK (tmp, MAX_LEN);
